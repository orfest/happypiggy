...
public class Shareholder extends Player implements IUShareholder {
	...
	private Map<Long, ShareHolding> shares = new HashMap<Long, ShareHolding>();	// keys are companyIds
	...
	public Shareholder(IUPlayer other) {
		super(other);
		// TODO init shareList, if bug #30 is fixed
		// also change `other` type
	}
	...
	public IUShareHolding subtractShareHolding(IUShareHolding toSub) {
		...
		if (sh == null || sh.getAmount() < toSub.getAmount())
			throw new IllegalArgumentException("Unable to subtract share holding");
	...
	public IUShareHolding getShareHoldingForCompanyId(long companyId) {
		...
		if (sh == null)
			return new ShareHolding(companyId, this.getId(), 0);
		return sh;
	}
